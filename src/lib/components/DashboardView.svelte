<script lang="ts">
  ...
  const WIDGET_LABELS: Record<DashboardWidget["widget_type"], string> = {
  ...
  async function saveLayout() {
    ...
    const newWidgets: DashboardWidget[] = localWidgetTypes.map((type) => {
      const pos = positions.get(type) ?? WIDGET_DEFAULTS[type];
  ...
  function addWidget(wt: DashboardWidget["widget_type"]) {
    ...
    const def = WIDGET_DEFAULTS[wt];
    positions.set(wt, { x: def.x, y: def.y, w: def.w, h: def.h, include_subprojects: def.include_subprojects });
    if (wt === "project_health") healthSubprojectsDraft = def.include_subprojects ?? false;
      ...
      <div class="gs-grid-wrapper">
        <div class="grid-stack" bind:this={gridEl}>
          {#each localWidgetTypes as wt (wt)}
            {@const pos = positions.get(wt) ?? WIDGET_DEFAULTS[wt]}
            <div
            ...
            >
              <div class="grid-stack-item-content widget-card edit-card">
                <div class="edit-handle">
                  ...
                  <span class="handle-label">{WIDGET_LABELS[wt]}</span>
                  ...
                  <button
                    ...
                    title="Remove widget"
                    aria-label="Remove {WIDGET_LABELS[wt]}"
                  >✕</button>
                </div>
          ...
          <div class="widget-library">
            {#each ALL_WIDGET_TYPES as wt (wt)}
              ...
              <div class="widget-lib-card" class:is-added={isAdded}>
                <span class="lib-name">{WIDGET_LABELS[wt]}</span>
                {#if isAdded}
                  <button
